...
if [[ -n "$ISSUE_NUMBER" ]]; then
  issue_json="$RUNNER_TEMP/cost-center-members-issue.json"
  issue_body="$RUNNER_TEMP/cost-center-members-issue.md"
  resolved_config_file="$RUNNER_TEMP/cost-center-members.yml"
  ...
  if ! jq -e 'any(.labels[].name; . == "cost-center-members-config")' "$issue_json" >/dev/null; then
    echo "ERROR: Issue #$ISSUE_NUMBER must have the cost-center-members-config label." >&2
    exit 1
  fi

  awk '
    BEGIN { in_section=0; in_fence=0 }
    /^### Cost center members YAML[[:space:]]*$/ { in_section=1; next }
  ...
  if [[ ! -s "$resolved_config_file" ]]; then
    echo "ERROR: Could not extract a fenced YAML block from the 'Cost center members YAML' issue field." >&2
    exit 1
  fi

  resolved_config_source="issue #$ISSUE_NUMBER: Cost center members YAML"
...

{
  ...
  echo "COST_CENTER_MEMBERS_CONFIG_FROM_ISSUE=$resolved_config_from_issue"
  echo "COST_CENTER_MEMBERS_ISSUE_NUMBER=$ISSUE_NUMBER"
  echo "COST_CENTER_MEMBERS_ISSUE_URL=$issue_url"
  echo "COST_CENTER_MEMBERS_ISSUE_UPDATED_AT=$issue_updated_at"
  echo "COST_CENTER_MEMBERS_CONFIG_SHA256=$config_sha256"
} >>"$OUTPUT_ENV_FILE"
